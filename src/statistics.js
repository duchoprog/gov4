...
function esperaLoco() {
    members.forEach((element, index) => stats[element.party].ids.push(index))
    ...
    function avgWP(partido) {
        ...
        for (i = 0; i < largor; i++) {
            ...
            wP += members[orden].votes_with_party_pct
        }
    ...
    var porFaltas = []
    function faltadores() {
        porFaltas = members.sort((a, b) => b.missed_votes - a.missed_votes)
        var i = 0
    ...
    porLoyalty = members.sort((a, b) => a.votes_with_party_pct - b.votes_with_party_pct)
